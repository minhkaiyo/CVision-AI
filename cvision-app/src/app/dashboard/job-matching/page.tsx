  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex items-start justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-11 h-11 rounded-2xl overflow-hidden bg-white/60 border border-white/80 flex items-center justify-center shrink-0 shadow-[inset_0_1px_2px_rgba(255,255,255,0.8)]">
            {imgError ? (
              <span className="text-slate-800 font-bold text-sm">{job.company[0]}</span>
            ) : (
              <Image
                src={job.logo}
                alt={job.company}
          ...
          <div className="min-w-0">
            <p className="text-[12px] text-slate-800 font-bold truncate flex items-center gap-1">
      ...
      <div className="relative z-10">
        <h3 className="font-extrabold text-slate-900 text-[15px] mb-2 group-hover:text-blue-700 transition-colors">
          {job.title}
        </h3>
        <p className="text-[12px] text-slate-700 font-medium leading-relaxed mb-2 line-clamp-2">{job.description}</p>
        <div className="flex flex-col gap-1.5">
          <div className="flex items-center gap-1.5 text-[12px] text-slate-700 font-semibold">
            <MapPin className="w-3.5 h-3.5 shrink-0 text-blue-500" /> {job.location}
          </div>
          <div className="flex items-center gap-1.5 text-[12px] text-slate-800 font-bold">
      ...
      <div className="relative z-10 flex flex-wrap gap-1.5">
        ...
        {job.tags.length > 4 && (
          <span className="text-[11px] font-semibold px-2 py-0.5 bg-slate-100 text-slate-600 border border-slate-200 rounded-lg">
            +{job.tags.length - 4}
          </span>
        )}
      </div>
      ...
      <div className="relative z-10 flex items-center justify-between pt-2 border-t border-white/40">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[10px] bg-slate-100/60 text-slate-600 px-2 py-0.5 rounded-md font-bold border border-slate-200">
            {job.source}
          </span>
        </div>
        <a
    ...
    <div className="space-y-6 pb-10">
      ...
      <div className={`${glassCard} px-7 py-6`}>
        ...
        <div className="relative z-10 flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-[24px] font-extrabold text-slate-900 flex items-center gap-2.5 mb-1 tracking-tight">
            ...
            <p className="text-slate-600 font-medium text-[13px] ml-12">
              {isPersonalized ? (
                <span className="flex items-center gap-1.5">
                  <CheckCircle className="w-3.5 h-3.5 text-emerald-500" />
                  Cá nhân hóa dựa trên CV: <strong className="text-slate-800">{cvAnalysis?.role}</strong>
                </span>
              ) : (
                <span className="flex items-center gap-1.5">
        ...
        <div className="relative z-10 flex flex-wrap gap-3 items-center">
          ...
          <div className="relative flex-1 min-w-[220px]">
            ...
            <input
              ...
              className="w-full pl-10 pr-10 py-2.5 text-[13px] text-slate-800 bg-white/50 border border-white/70 rounded-2xl backdrop-blur-sm focus:outline-none focus:border-blue-300 focus:bg-white/70 transition font-bold placeholder:text-slate-500"
            />
            {search && (
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            {WORK_TYPES.map(t => (
              <button
                ...
                className={`px-3 py-2 rounded-xl text-[12px] font-bold transition-all border ${workType === t.id ? "bg-blue-500 text-white border-blue-400 shadow-md" : "bg-white/40 text-slate-700 border-white/60 hover:bg-white/60"}`}
              >
                {t.label}
              </button>
            ))}
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-[12px] font-bold transition-all border ${showFilters ? "bg-indigo-500 text-white border-indigo-400" : "bg-white/40 text-slate-700 border-white/60 hover:bg-white/60"}`}
          >
              ...
              <div className="pt-3 mt-3 border-t border-white/40 flex flex-wrap gap-3 items-center">
                ...
                {SALARY_FILTERS.map(s => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl text-[12px] font-bold border transition-all ${salaryFilter === s.id ? "bg-emerald-500 text-white border-emerald-400" : "bg-white/40 text-slate-700 border-white/60 hover:bg-white/60"}`}
                  >
                    {s.label}
                  </button>
                ))}
